...
Before handoff or deployment, run the same checks used by GitHub Pages:
...
- This is a static client-side app: it has no backend, API, database, account system, or telemetry.
- Answers are stored only in the browser's localStorage so an interrupted questionnaire can resume. If browser storage is unavailable, the app stays usable and clearly says that progress will not survive a refresh.
- Dietary filters only apply to ingredients explicitly tagged in the local style dataset. They are not an allergy guarantee; always confirm ingredients and cross-contamination with the restaurant.
- The `main` branch deploys to GitHub Pages through `.github/workflows/deploy.yml` after a clean install, lint, tests, and production build.
...
3. 視需要補更完整的整體流程測試，例如更多真實裝置上的 keyboard / screen-reader 回歸。
