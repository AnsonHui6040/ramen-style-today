...
export function getPopulatedFinderRegions(regions: readonly FinderRegion[]) {
  return regions.filter(
    (region) => Number.isSafeInteger(region.shopCount) && region.shopCount > 0,
  )
}
...
export function toSafeHttpsUrl(value: unknown) {
...
function isHttpsUrl(value: string) {
  return toSafeHttpsUrl(value) !== null
}
