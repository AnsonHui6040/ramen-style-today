...
describe('ramen map domain', () => {
  ...
  test('only returns absolute HTTPS URLs for outbound links', () => {
    expect(toSafeHttpsUrl('https://example.com/menu?lang=zh-TW')).toBe(
      'https://example.com/menu?lang=zh-TW',
    )
    expect(toSafeHttpsUrl('http://example.com')).toBeNull()
    expect(toSafeHttpsUrl('javascript:alert(1)')).toBeNull()
    expect(toSafeHttpsUrl('//example.com')).toBeNull()
    expect(toSafeHttpsUrl('not a url')).toBeNull()
  })

  test('only exposes regions with published shop coverage', () => {
    const regions: FinderRegion[] = [
      {
        region: '台北市',
        regionCode: 'taipei',
        shopCount: 0,
        dataPath: '/taipei.json',
      },
      {
        region: '台中市',
        regionCode: 'taichung',
        shopCount: 30,
        dataPath: '/taichung.json',
      },
      {
        region: '高雄市',
        regionCode: 'kaohsiung',
        shopCount: -1,
        dataPath: '/kaohsiung.json',
      },
    ]

    expect(getPopulatedFinderRegions(regions)).toEqual([regions[1]])
  })
})
